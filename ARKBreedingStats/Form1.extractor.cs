...
namespace ARKBreedingStats
{
    public partial class Form1
    {
        ...
        private bool ExtractLevels(bool autoExtraction = false, bool statInputsHighPrecision = false, bool showLevelsInOverlay = false, Creature existingCreature = null, bool possiblyMutagenApplied = false)
        {
            ...
            while (true)
            {
                ...
                if (bred && numericUpDownImprintingBonusExtractor.Value == 0 && statInputsHighPrecision)
                {
                    ...
                    if (!someStatsHaveNoResults || !onlyStatsWithTeHaveNoResults)
                    {
                        break;
                    }
            ...
            for (int s = 0; s < Stats.StatsCount; s++)
            {
                ...
                else
                {
                    ...
                    if (s == Stats.SpeedMultiplier && (rbBredExtractor.Checked || !RbBondedTaming0.Checked) && numericUpDownImprintingBonusExtractor.Value > 0)
                    {
        ...
        {
            if (!Properties.Settings.Default.MoveMutationLevelsOnExtractionIfUnique || CreatureCollection.CurrentCreatureCollection?.Game == Ark.Ase)
            {
                return false;
            }
        ...
        private void SetCreatureValuesToExtractor(CreatureValues cv, bool setInfoInput = true)
        {
            ...
            if (cv.isBred && cv.imprintingBonus > 0.01 && cv.imprintingBonus - Ark.ImprintingPerBondedTamingRank(BondedTamingRankExtractor) < 0.00001)
            {
                cv.isTamed = true;
                cv.isBred = false;
            }
        ...
        private int BondedTamingRankExtractor
        {
            get
            {
                if (RbBondedTaming3.Checked)
                {
                    return 3;
                }

                if (RbBondedTaming2.Checked)
                {
                    return 2;
                }

                if (RbBondedTaming1.Checked)
                {
                    return 1;
                }
