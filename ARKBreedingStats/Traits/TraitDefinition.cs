    ...
    public static class TraitDefinitionLoader
    {
        public static void LoadTraitDefinitions()
        {
            ...
            foreach (var t in defs)
            {
                ...
                {
                    if (string.IsNullOrEmpty(traitDef.Name))
                    {
                        traitDef.Name = baseTrait.Name;
                    }

                    if (string.IsNullOrEmpty(traitDef.Description))
                    {
                        traitDef.Description = baseTrait.Description;
                    }

                    if (string.IsNullOrEmpty(traitDef.Effect))
                    {
                        traitDef.Effect = baseTrait.Effect;
                    }

                    if (traitDef.MutationProbability == null)
                    {
                        traitDef.MutationProbability = baseTrait.MutationProbability;
                    }

                    if (traitDef.InheritHigherProbability == null)
                    {
                        traitDef.InheritHigherProbability = baseTrait.InheritHigherProbability;
                    }

                    if (traitDef.MaxCopies == -1)
                    {
                        traitDef.MaxCopies = baseTrait.MaxCopies;
                    }
                }
