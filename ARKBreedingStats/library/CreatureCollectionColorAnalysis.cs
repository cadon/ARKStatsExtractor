...
namespace ARKBreedingStats.Library
{
    ...
    internal static class CreatureCollectionColorAnalysis
    {
        ...
        internal static ColorStatus[] DetermineColorStatus(this CreatureCollection cc, Species species, byte[] colorIds, out string infoText, out int[][] creaturesWithColorsInRegion, out bool[] desiredColors)
        {
            ...
            desiredColors = null;
            if (string.IsNullOrEmpty(species?.blueprintPath) || colorIds == null)
            {
                return null;
            }
            ...
            if (anyColorNew)
            {
                ColorFlagsCombined |= ColorStatus.NewColor;
            }

            if (anyColorNewInRegion)
            {
                ColorFlagsCombined |= ColorStatus.NewRegionColor;
            }

            if (desiredColors.Any(ci => ci))
            {
                ColorFlagsCombined |= ColorStatus.DesiredColor;
            }
