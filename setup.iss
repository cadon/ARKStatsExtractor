...
de.WelcomeLabel2=Dieser Assistent wird jetzt [name/ver] auf Ihrem Computer installieren.%n%nWenn Sie planen [name] als portable Version in einem gemeinsam genutzten Verzeichnis (das heiﾟt, auﾟerhalb des Verzeichnisses f・ Programme) auszuf・ren, empfehlen wir anstelle dieses Installationsprogramms die Zip-Datei-Version zu nutzen.

[CustomMessages]
DotNetFrameworkNeededCaption=.NET Framework 4.8 required
de.DotNetFrameworkNeededCaption=.NET Framework 4.8 benigt
DotNetFrameworkNeededDescription=To run {#AppName} the .NET Framework 4.8 is required.
de.DotNetFrameworkNeededDescription=Um {#AppName} auszuf・ren wird .NET Framework 4.8 benigt.
DotNetFrameworkNeededSubCaption=Check the box below to download and install .NET Framework 4.8.
de.DotNetFrameworkNeededSubCaption=Markieren Sie das folgende K舖tchen, um .NET Framework 4.8 herunterzuladen und zu installieren.
...
de.IDP_DownloadFailed=Herunterladen von .NET Framework 4.8 fehlgeschlagen. .NET Framework 4.8 wird benigt um {#AppName} auszuf・ren.
...
de.DotNetFrameworkFailedToLaunch=Starten des .NET Framework Installer fehlgeschlagen mit Fehler "%1". Bitte den Fehler beheben und dieses Installationsprogramm erneut ausf・ren.
...
de.DotNetFrameworkFailed1602=Die .NET Framework Installation wurde abgebrochen. Diese Installation kann fortgesetzt werden. Beachten Sie jedoch, dass diese Anwendung mlicherweise nicht ausgef・rt wird, bis die .NET Framework-Installation erfolgreich abgeschlossen wurde.
...
de.DotNetFrameworkFailed1603=Ein schwerwiegender Fehler trat w臧rend der Installiion des .NET Frameworks auf. Bitte den Fehler beheben und dieses Installationsprogramm erneut ausf・ren.
DotNetFrameworkFailed5100=Your computer does not meet the requirements of the .NET Framework.
de.DotNetFrameworkFailed5100=Ihr Computer erf・lt nicht die Voraussetzungen f・ das .NET Framework.
...
de.DotNetFrameworkFailedOther=Die .NET Framework Installation endete mit dem nicht erwarteten Statuscode "%1". ﾜberpr・en Sie alle anderen vom Installationsprogramm angezeigten Meldungen, um festzustellen, ob die Installation erfolgreich abgeschlossen wurde, und falls nicht, brechen Sie die Installation ab und beheben Sie das Problem.
DownloadImages=Download images for some of the species (~13 MB).
de.DownloadImages=Bilder f・ einige der Dinos herunterladen (~13 MB).
CreatureImages=Additional creature images
de.CreatureImages=Zus舩zliche Dino-Bilder
InstallImages=Installing additionally creature images
de.InstallImages=Installiere zus舩zliche Dino-Bilder
